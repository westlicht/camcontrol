...
static QState Tunnel_screen_saver_n_pixels(Tunnel *me);
static QState Tunnel_screen_saver_1_pixel (Tunnel *me);
...
QState Tunnel_screen_saver(Tunnel *me) {
    switch (Q_SIG(me)) {
        case Q_INIT_SIG: {
            return Q_TRAN(&Tunnel_screen_saver_n_pixels);
        }
        case PLAYER_TRIGGER_SIG: {
            return Q_TRAN(&Tunnel_demo);
        }
...
/*..........................................................................*/
QState Tunnel_screen_saver_n_pixels(Tunnel *me) {
    switch (Q_SIG(me)) {
        case Q_ENTRY_SIG: {
            QActive_arm((QActive *)me, BSP_TICKS_PER_SEC*60*1);   /* 60 sec */
            /* clear the screen */
            memset(l_frame, (uint8_t)0,
                   (GAME_SCREEN_WIDTH * GAME_SCREEN_HEIGHT/8));
            return Q_HANDLED();
        ...
        case Q_TIMEOUT_SIG: {
            return Q_TRAN(&Tunnel_screen_saver_1_pixel);
        }
        case TIME_TICK_SIG: {
...
/*..........................................................................*/
QState Tunnel_screen_saver_1_pixel(Tunnel *me) {
    switch (Q_SIG(me)) {
        case Q_ENTRY_SIG: {
            /* clear the screen frame buffer */
            memset(l_frame, (uint8_t)0,
                   (GAME_SCREEN_WIDTH * GAME_SCREEN_HEIGHT/8));
            me->blink_ctr = (uint8_t)0;       /* used as index to l_frame[] */
            QActive_arm((QActive *)me, BSP_TICKS_PER_SEC/5);/* 1/5s timeout */
            return Q_HANDLED();
        }
        case Q_EXIT_SIG: {
            QActive_disarm((QActive *)me);
        ...
        case Q_TIMEOUT_SIG: {
            ...
            l_frame[me->blink_ctr] = (uint8_t)0;   /* extinguish last pixel */

                                                  /* light up another pixel */
            me->blink_ctr = rnd % (GAME_SCREEN_WIDTH * GAME_SCREEN_HEIGHT/8);
            l_frame[me->blink_ctr] = (uint8_t)(1 << ((rnd >> 8) & 0x7));

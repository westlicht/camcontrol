...
PARAM_CLASS(pc_overlap, UINT16, 10, 5000, 1, 100, print_overlap)

/* Camera parameters */
PARAM(p_focal_length, &pc_focal_length, &pd.camera.focal_length, 180)
PARAM(p_sensor_width, &pc_sensor_dim, &pd.camera.sensor_width, 240)
PARAM(p_sensor_height, &pc_sensor_dim, &pd.camera.sensor_height, 360)
PARAM(p_crop, &pc_crop, &pd.camera.crop, 100)

/* Servo parameters */
PARAM(p_servo_min_x, &pc_servo_range, &pd.servo.min_x, 75)
PARAM(p_servo_max_x, &pc_servo_range, &pd.servo.max_x, 752)
PARAM(p_servo_min_y, &pc_servo_range, &pd.servo.min_y, 164)
PARAM(p_servo_max_y, &pc_servo_range, &pd.servo.max_y, 718)

/* Misc parameters */
PARAM(p_misc_backlight, &pc_backlight, &pd.misc.backlight, 5)
...
PARAM(p_giga_overlap, &pc_overlap, &pd.giga.overlap, 500)
